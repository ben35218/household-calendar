...
- **P5 — location encryption + client weather** (D2 as-built). *Decided 2026-07-06: encrypt `Household` location + move weather client-side.*
  ...
  - **P5b (client-side weather)** ⏳ **remaining.** Backend is **open-meteo** (keyless + CORS-open), so the client can geocode + fetch forecasts **directly** — port `services/weather.js` (geocode + `buildForecast`) to a client/shared module, fetch from the decrypted coords, drop the server `WeatherRecord` cache post-drop. Also wire calendarChat's `get_weather_forecast` to client-provided forecast data.
