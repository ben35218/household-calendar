...
- **"Shared" = a `shareCode` exists OR ≥1 collaborator has joined.** Disabling sharing clears both. Centralized in `services/tripSharing.js` (`isTripShared`, `sharedTripIds`, `excludeSharedFilter`; unit-tested).
...
- **Deferred (post-drop only):** the **decrypt-on-share** client step. Post-drop a *private* trip is ciphertext-only; to share it the owner's device must decrypt + re-save the trip and its items as plaintext (clearing `enc`) before the share code goes live. Until that's built, `POST /trips/:id/share` **fails safe** on an `e2eeActive` household with `409 decrypt_required` rather than exposing unreadable data. Also deferred: enforcing the invariant on *steady-state edits* to an already-shared trip (today the drop-exempt + fail-safe guard cover correctness; a server-side "strip `enc` when the trip is shared" write rule would make it robust against a re-seal).
