...
- **P6 — close dual-write field/collection gaps.** (User chose *full* P6.) *Key finding: `CalendarEvent` already seals its whole payload (dates included) — only the plaintext column was kept for the server date filter.*
  ...
  - **P6b** ⏳ **remaining (the hard slice):** encrypt `MaintenanceTask`/`Chore.nextDueDate` → the server `tasks.js` status filters (`overdue`/`due-soon`/`upcoming`) can no longer run, so the **maintenance status dashboards must bucket client-side** on web **and** mobile. Multi-screen rearchitecture.
  - **P6c** ⏳ **remaining:** dual-write for the deferred thin collections (`RecipeSchedule`, `OdometerLog`, template-created tasks/chores). `Category` stays plaintext (decided). `Household` location done in P5a.
