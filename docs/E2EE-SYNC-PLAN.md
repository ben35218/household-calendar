...
> **NOTE (2026-07-07):** The consumer **web app (`client/`) has been removed** — the only clients are now the native mobile app (`mobile/`) and the Vue admin web app (`admin/`). References below to the web client / `client/…` are historical; the E2EE work continues in `mobile/` (and `shared/`) only.

---

## CURRENT STATUS SNAPSHOT (2026-07-06)

> **UPDATE (2026-07-08) — verification-debt + last-code pass; all committed on `e2ee-sync-checkpoint`:**
> - **Server integration-test harness** (`src/test/harness.js`: mongodb-memory-server + supertest over the real app, split out as `src/app.js`). **The previously never-executed paths now run in CI-able tests (server suite 100):** HDK mint/join/removal/rotation incl. the compare-and-set race and version-map envelopes; §9.3 decrypt-on-share + steady-state guards; and the **full §9 drop journey** — straggler seal via the API, min-app-version gate, dry run, `--commit`, shared-trip exemption, then the API exercised live with `e2eeActive=true`. `dropPlaintext.js` refactored to an exported function + thin CLI (same exit codes).
> - **Two real bugs found & fixed by those tests:** (1) `requireTripAccess` selected only `_id userId`, so `isTripShared()` was always false in the trip-item write guards — stale ciphertext could ride onto a shared booking; (2) the manuals **multer fileFilter rejected `application/octet-stream`**, which is what encrypted uploads send — encrypted manual upload would have failed on first device run.
> - **Phase 4c closed out:** TripItem attachments — server subdoc crypto fields + encrypted upload (private bookings only; 409 on shared, §9.3-consistent) + download-as-octet-stream; **new mobile Attachments UI** in TripItemFormScreen (attach/open/remove, encrypt/decrypt via `lib/attachments.ts`). *Item photos + receipt images confirmed ephemeral (memory-storage vision extraction, never persisted) — nothing to wire.*
> - **Passkeys (react-native-passkeys, PRF):** `shared/crypto` `addPasskey`/`unlockWithPasskeyPrf` (+1 test, 24); mobile ceremonies (`lib/passkeys.ts`), AccountScreen "Passkey unlock" card, and an automatic Face ID unlock attempt on token-restore relaunch. Requires: EAS rebuild, iOS 18+/Android PRF, and the RP-ID domain serving AASA/assetlinks + `ios.associatedDomains` (config: `PASSKEY_RP_ID`).
> - **Background reminder refresh:** `expo-background-fetch`+`expo-task-manager` task re-runs `rescheduleReminders`; registered by `useReminderScheduler`; `UIBackgroundModes: fetch`.
> - **Encrypted-backup restore UI:** "Restore from backup…" in PrivacyScreen (picker + passphrase → `importEncryptedBackup`).
> - **§9.1 P4 polish:** all three chat `GET /context` endpoints also accept **POST with client-supplied decrypted records**; mobile assistants send them and refresh the panel.
> - **Release path:** see `docs/RELEASE-SMOKE-CHECKLIST.md` for the one EAS build + on-device smoke pass + per-household drop runbook. Verification bar: crypto 24, calendar 9, weather 6, server **100** (incl. 16 integration), mobile tsc 0, expo export ios — all green.
>
> **UPDATE (2026-07-08, later) — auth hardening pass (working tree):** passkeys are now also a SIGN-IN credential, not just an unlock factor. `addPasskeyFactor` runs a server-verified WebAuthn registration (`routes/authPasskey.js`, `@simplewebauthn/server`; credential public keys on `User.passkeyCredentials`), and `/auth/passkey/challenge` returns each credential's `prfSalt` so a single Face ID assertion signs in AND unlocks E2EE (`store/auth.loginWithPasskey`). Forgot-password (emailed 6-digit code, `/auth/forgot`+`/auth/reset`) deliberately leaves the stale password envelope in place — the client re-wraps via `rewrapForNewPassword` after a passkey/recovery unlock. Plus per-IP rate limits on the auth endpoints and a sliding JWT refresh (`X-Refreshed-Token`). New config: `PASSKEY_ORIGINS` (defaults to `https://<PASSKEY_RP_ID>`; add the Android `apk-key-hash` origin for Play builds). Passkeys enrolled before this pass have no stored public key → no sign-in until re-added from Security. Server suite 119 (+11 auth-flow integration tests).
...
**Remaining-work completion pass (2026-07-07, build/test-verified — mobile `tsc` + `expo export ios` green, server suite 76):**
- **Phase 7 — member removal + lazy HDK rotation (§5.2).** `Household.keyRotationPending`; server `POST /household/members/:id/remove` (owner-only → departed member to a fresh solo household + rotation flagged via `handleDeparture`, which now also drops the departed member's stale envelopes), `POST /household/key/rotate` (compare-and-set version bump + new-version envelopes for every enrolled member; old versions kept for historical reads), `GET /household/member-keys`; `validateRotation` shape-validator (+4 tests). Client `e2ee.ts` now holds a **version→HDK map** (reads historical records after a rotation) and self-heals `rotateHouseholdKey` from `ensureHouseholdKey` when the flag is set; `HouseholdScreen` gains an owner "remove member" action. *Deferred (as before): re-encrypting a **departed** member's own moved data under their new solo HDK — the cross-household migration concern; historical-read access is the accepted §5.2 limitation.*
- **§9.3 decrypt-on-share — built.** `POST /trips/:id/share` now accepts the client-decrypted `{ trip, items }` and re-writes them as plaintext (clearing `enc`) before minting the share code; the mobile share action catches `409 decrypt_required`, decrypts via `openRecord`, and retries. Steady-state guard added: trip/item writes **strip `enc` when the trip is shared** (`isTripShared`), so an edit can't re-introduce ciphertext a collaborator can't read.
- **Phase 5 AI transparency — extended to all surfaces.** Shared `ChatScreen` now hard-gates on `aiEnabled` (assistant unusable when off) — covers calendar/maintenance/vacation assistants; new reusable `AiConsentNote`; recipe import, receipt scan, and manual task-extract all gated + annotated.
- **Phase 5a notifications toggle.** `remindersEnabled` pref + Privacy "Reminders" switch; scheduler + `rescheduleReminders` honor it (cancel-all when off). *Background-refresh still needs `expo-background-fetch`+`expo-task-manager` (native dep) — see §11 Phase 5.*
- **§6.4 local→household transition.** Shared `cancelDeletionSet()`; join-approval now cancels a joiner's pending local purge + puts them back on cloud (members can't be local), with a `deletion_canceled` audit.
- **Phase 4c mobile-full (manuals).** `expo-file-system` installed; `lib/e2ee` `encryptAttachment`/`decryptAttachment` + `lib/attachments.ts` (chunked file crypto + base64 IO); `ItemDetailScreen` encrypts on upload and decrypts-to-temp + shares on open (replacing "open on web"). *Item photos / receipt images / TripItem attachments follow the same `attachments.ts` pattern — not yet applied.*
- **Phase 4b expo-sqlite (D5).** `lib/sqliteReplica.ts` implements the `ReplicaStore` interface over `expo-sqlite`; `replica.ts` prefers it and **falls back to AsyncStorage** when the native module isn't linked yet — auto-upgrades after a dev-client rebuild.
- **Decision 12 encrypted export/import.** `lib/exportData.ts` — passphrase (Argon2id KEK via `createPasswordFactor`) wraps a random export key that chunk-AEAD-encrypts the whole replica to a `.hcbackup` file; Privacy screen "Export encrypted backup" (iOS `Alert.prompt` + Share); `importEncryptedBackup` restores into the replica.

*Still requires the user (per "write code, you run"): the actual prod `dropPlaintext --commit`, native **dev-client rebuild** to run the new `expo-file-system`/`expo-sqlite` code on-device, SMTP + `CLOUD_PURGE_LIVE` for real purge, background-refresh native deps, and **mobile passkey** (`react-native-passkeys`). On-device verification of everything below the flag is still pending — nothing has run with `e2eeActive = true`.*

**Not started / blocked / gated:**
- **§9 plaintext drop** — the real milestone (records actually private). **All dormant prerequisites P1–P7 are now resolved** (§9.1): P1 (onboarding self-Person→client), P2 (shared calendar engine, client expansion), P3 (reminder cron gated + client engine), P4a–d (all five AI surfaces run without server plaintext, incl. the one write-tool creating encrypted client-side), P5a/b (home location sealed + client weather over open-meteo), P6a (Person.birthday encrypted + calendar date filters relocated) — all **build/test-verified & dormant behind `Household.e2eeActive`**. P6b (`nextDueDate`) + P6c (thin collections) are **grounded keep-plaintext decisions** (server-computed/validated fields; §10 metadata). P7 (replica ciphertext) is **already satisfied** by dual-write. **The drop itself is the only remaining step and needs live/on-device verification** (nothing has run with the flag on) — see §9.2.
...
- **Phase 6 — FIRST SLICE DONE** (2026-07-07; build/test-verified, dry-run). Storage-mode toggle now wired end-to-end on mobile: server `User.storageMode`/`cloudDeletion*`/`localReplicaVerified*` fields (§4.1) + sparse purge index; pure `services/cloudDeletion.js` (7-day date math, due predicate, solo guard, download-first manifest compare via a mirrored non-crypto `cyrb53` fingerprint; **7 tests** + a fixed-vector pin → server suite **36**); `routes/storage.js` (`GET /storage`, `POST /switch-to-local` = verify-manifest-then-schedule atomically, `POST /switch-to-cloud` = undo); `services/mailer.js` (nodemailer, `isConfigured()` guarded no-op when SMTP unset — scheduled/canceled/purged templates); AuditLog lifecycle events; mobile `replicateAndBuildManifest` (download-first over 8 collections into the replica), blocking confirmation dialog, solo-only guard (§6.1), persistent app-wide countdown banner + 7-day undo. **Deferred (unverifiable here):** the destructive purge is **dry-run only** — the sweep cron logs what it *would* delete and changes nothing unless `CLOUD_PURGE_LIVE==='true'` (flip on only after staging + a fuller replica incl. attachments); real email *send* (guarded no-op today); web client UX. See §6 / §11.
- **Phase 7** — member removal / HDK rotation + whole-household migration. Not started.

**Verification bar (keep green):** `cd shared/crypto && npm test` (23) · `cd shared/calendar && npm test` (9) · `cd shared/weather && npm test` (6) · `cd server && npm test` (40) · `cd client && npm run build` · `cd mobile && npx tsc --noEmit` · `cd mobile && CI=1 npx expo export --platform ios`. On-device optional (Metro dev server + iOS sim).

**Follow-on slice (2026-07-07; build/test-verified, all dormant/pre-drop):**
- **P5 secondary weather → client-side.** `shared/weather` gained `fetchWeatherArchive` + pure `buildRangeRecords`/`buildOutlook` + `loadWeatherRange`/`loadOutlook` (6 tests). Web calendar overlay (`CalendarView` → `loadWeatherRangeData`) + web 90-day (`WeatherView`) + mobile 90-day (`WeatherScreen`) now fetch client-direct over the decrypted address when `e2eeActive && HDK`, else the server. The server `WeatherRecord` cache is bypassed for E2EE households (retired-by-disuse; stays for non-E2EE during dual-write).
- **"Sent to Anthropic" consent indicator** on all chat surfaces — one line in the shared web `ChatPanel.vue` + mobile `ChatScreen.tsx` covers calendar/maintenance/vacation assistants on both platforms.
- **§9 drop go-live buildables:** **min-app-version readiness signal** (`computeReadiness` version gate + `compareVersions`/`versionSatisfied`, `E2EE_MIN_APP_VERSION` env, `User.clientVersion` reported on login web+mobile, `POST /household/e2ee/client-version`); **client straggler re-encrypt pass** (`GET /household/e2ee/stragglers` + content-blind `POST /household/e2ee/seal` + client `dropMigration` `reencryptStragglers`); **in-app migration UI** (web `E2eeMigrationView` + mobile `E2eeMigrationScreen`: readiness checklist + re-encrypt, entry points in both profile hubs).
- **Web storage-mode UX** (rounds out Phase 6): web `storageMode.js`/`storageState.js` + `StorageBanner.vue` (mounted in `App.vue`) + `StorageSection.vue` (profile hub), mirroring the mobile download-first → schedule → undo flow over the IndexedDB replica. Purge still dry-run.
...
> **Front door is now an email invitation, not a code (updated 2026-07-12).** A member invites an email (`POST /household/invitations`, emailed via `sendHouseholdInvitation`); the recipient accepts from their Invitations inbox (`POST /household/invitations/:id/accept`), which opens the `JoinRequest` in step 1 below and links it to the invitation. The cryptographic approve-on-device step (2–5) is unchanged — the invitation only replaces code-based discovery/authorization. **`joinCode` is fully removed** — the field is gone from the `Household` model and every admin view, `POST /household/join` is deleted, and `db.js` drops the legacy `joinCode` unique index on boot (a missing-field null would otherwise collide under that index).

1. Joiner accepts an invitation (the household join code is gone entirely). A `JoinRequest` is created with the joiner's `identityPublicKey`.
...
> **Status (2026-07-07): first slice built (mobile), dry-run.** §6.1 solo guard, §6.2 download-first + schedule, §6.3 undo, and the §6.2 banner/emails/AuditLog are implemented and build/test-verified. The one thing that does **not** run is the irreversible delete in §6.2 step 6 — the sweep cron is dry-run until `CLOUD_PURGE_LIVE==='true'` (flip on only after staging + attachment/full-replica coverage). See §11 Phase 6.
...
- **P4 — AI routes → ephemeral-consent** for the remaining surfaces (calendar/maintenance/vacation chat, recipe/receipt scan, manual-extract). *Sliced (2026-07-06): P4a read-only chats → P4b scan/extract → P4c calendarChat agentic tool execution. Mechanism = routes accept optional client-supplied decrypted records; use them when present, else read the DB (dual-write fallback).*
  - **P4a** ✅ **DONE** (build/test-verified; dormant). Server: `vacationChat` POST accepts optional decrypted `trip`+`items` (nav-only tools → **fully** drop-capable); `maintenanceChat` POST accepts optional decrypted `item` for the system prompt; both verify `tripId`/`itemId` access via plaintext metadata, else fall back to the DB (behavior unchanged today). Clients: web (`VacationAssistantView`/`MaintenanceChatView`) + mobile (`VacationAssistantScreen`/`MaintenanceChatScreen`) fetch + decrypt the records and send them in `buildBody` when `e2eeActive && HDK held`. *Remaining: the `GET /…/chat/context` summary endpoints still read the DB (minor post-drop degrade); a "sent to Anthropic" indicator; `maintenanceChat`'s manuals-text + `get_item_tasks`/`create_tasks` tools still hit the DB (→ P4c). Then **P4b** scan/extract, **P4c** calendarChat agentic tool execution.*
  - **P4c** ✅ **DONE** (build/test-verified; dormant). calendarChat's only server-plaintext reads were the **people** context (system prompt) and the **read** tools `list_events` (→ collectCalendarRecords) + `call_business` (event phone) — its create/edit/delete tools only return `navigateTo` prefilled forms (the user drives the write, which already encrypts client-side), so **no bidirectional tool-execution redesign was needed**. Server POST now accepts optional decrypted `people` + `calendarSources`; `list_events` expands the supplied sources with the shared `assembleCalendarData` (same engine), `call_business` finds the event in them; DB fallback otherwise. Clients: web `CalendarAssistantView` + mobile `CalendarAssistantScreen` fetch a wide window of decrypted sources (`loadCalendarSources`) + the decrypted roster and send them when `e2eeActive && HDK`.  - **P4d** ✅ **DONE** (build/test-verified; dormant). `maintenanceChat`'s `create_tasks` — the one genuine server-side *write* tool — now, when `household.e2eeActive`, returns the computed task payloads instead of writing; the web/mobile assistants create them **encrypted** (`sealNew`) in `onResult`, and send the item's decrypted `tasks` so `get_item_tasks` sees real titles. DB write + read otherwise (unchanged today). *P4 is now functionally drop-complete; only non-blocking polish remains: the `GET /…/chat/context` summary endpoints (degrade to empty summaries post-drop) and a "sent to Anthropic" transparency indicator (mobile 5b pattern).*
  - **P4b** ✅ **DONE** (build/test-verified). Recipe AI (`from-photo`/`from-url`/`from-ai`/`edit-with-ai`) is **already ephemeral** — it operates on user-supplied input (uploaded image, URL, description, client-provided recipe), never a stored read, so no drop change is needed (only a consent indicator remains, UI polish). Manual task-extraction: `manualParser` split into a buffer path (`parseManualBufferForTasks`), and `POST /manuals/:id/extract-tasks` now accepts an in-memory (never-stored) decrypted PDF; web `ItemDetailView` decrypts an encrypted manual on device and posts the bytes ephemerally. Plaintext manuals extract server-side as before. *(Mobile manuals stay "open on web" per 4c.)*
- **P5 — location encryption + client weather** (D2 as-built). *Decided 2026-07-06: encrypt `Household` location + move weather client-side.*
  - **P5a (location encryption)** ✅ **DONE** (build/test-verified; dormant). `Household` gains `...encFields`; `PUT /settings` accepts + `GET /settings` returns the sealed location blob (+ `householdId`/`keyVersion` for AAD). Web `useProfileForm` + mobile `AccountScreen` seal `homeAddress` on save and decrypt it on load when the HDK is held. Name/joinCode/plan/timezone stay plaintext metadata. Dual-write: plaintext location still authoritative today.
  - **P5b (client-side weather)** ✅ **DONE for the main forecast** (build/test-verified; decided = client-direct geocode via open-meteo). New pure/dependency-free `shared/weather` package (`buildForecast` + open-meteo `geocode`/`fetchWeather`/`loadWeatherForAddress`; 4 `node:test`, uses global `fetch`). Client `loadForecast()` (web `services/weather.js` + mobile `lib/weather.ts`): when `e2eeActive && HDK`, decrypt `homeAddress` → geocode → open-meteo forecast → `buildForecast`; else the server forecast (no extra requests when locked). Wired: web `WeatherWidget` + `CalendarDayView`, mobile `WeatherScreen` + `CalendarDayScreen`. Packaging: vite `commonjsOptions` regex generalized to `shared/(calendar|weather)`, metro `extraNodeModules`. *Follow-ups — DONE (2026-07-07, build/test-verified): `shared/weather` gained `fetchWeatherArchive` + pure `buildRangeRecords`/`buildOutlook` + `loadWeatherRange`/`loadOutlook` (6 tests); web calendar overlay (`CalendarView`), web + mobile 90-day outlook now client-direct over the decrypted address when `e2eeActive && HDK`, else server. `WeatherRecord` cache is bypassed for E2EE households (retired-by-disuse; kept for non-E2EE during dual-write). calendarChat `get_weather_forecast` was already client-supplied.*
- **P6 — close dual-write field/collection gaps.** (User chose *full* P6.) *Key finding: `CalendarEvent` already seals its whole payload (dates included) — only the plaintext column was kept for the server date filter.*
  - **P6a** ✅ **DONE** (build/test-verified). `Person.birthday` added to the encrypt subset (web + mobile); `fetchCalendarSources`' `startDate`/`birthday` DB filters gated on `e2eeActive` (post-drop `GET /calendar/raw` returns all events/people, client filters via the shared engine; pre-drop unchanged).
  - **P6b — `nextDueDate` stays plaintext scheduling metadata** (decided 2026-07-06, grounded). It is **server-computed across the whole task lifecycle** (`tasks.js`/`chores.js` create, `POST /:id/complete`, from-template) *and* drives the dashboard status filters, so encrypting it is a task-lifecycle rearchitecture (client-compute on create+complete + client-side dashboard bucketing on both apps). Its sensitivity is *timing*, not content (titles are encrypted) — the same category as `reminderAt`/timestamps the plan already accepts as plaintext (§10). Revisit only if maintenance-schedule timing must be hidden; then do the full lifecycle move as its own effort.
  - **P6c — thin collections stay plaintext** (decided 2026-07-06, grounded). `OdometerLog.reading` is server-validated (must exceed the prior reading) and feeds server-side mileage estimation (`avgKmPerDay`/`estimateDateFromKm`), so it can't be encrypted without moving that logic too; the rest is refs/dates/servings (routing) + a minor `notes` field — encrypting `notes` beside an exposed `reading` adds negligible privacy. `RecipeSchedule` is recipeId + date + servings + a minor `notes`. `Category` stays plaintext (decided). Matches the doc's original "thin metadata / low value" deferral. `Household` location was done in P5a.
- **P7 — replica at-rest = ciphertext.** ✅ **Already satisfied by the dual-write architecture** (no correctness code needed). The replica upserts the raw server records and reads them through `openRecord`; post-drop the server returns `enc`-only records (plaintext columns nulled), so the replica stores ciphertext at rest and decrypts in memory on read automatically. *Remaining is optional perf/scale only: swap the mobile AsyncStorage backend for **expo-sqlite** (D5) — a native dep + dev-client rebuild, not a drop blocker.*
...
**To actually go live:** (1) hit the readiness endpoint / run the dry run; (2) owner's device re-encrypts any stragglers (records lacking `enc`); (3) run the dry run again → 0 stragglers; (4) **on a staging copy, `--commit` then verify on-device** that an unlocked member still reads everything; (5) then commit in prod. **Now built (2026-07-07, build/test-verified — needs live exercise):** the **min-app-version signal** (`E2EE_MIN_APP_VERSION` env → `computeReadiness` gate; clients report `User.clientVersion` on login), the in-app **migration UI** (web `E2eeMigrationView` + mobile `E2eeMigrationScreen`: readiness checklist + re-encrypt button), and the client **straggler re-encrypt pass** (`GET /household/e2ee/stragglers` → seal under HDK → content-blind `POST /household/e2ee/seal`). The `--commit` script itself is still operator-run. Live-verification checklist once flipped: calendar expansion over `/calendar/raw` (all-dates path), on-device reminders, all five AI surfaces with client-supplied context, encrypted-manual extract, client weather via open-meteo (incl. calendar overlay + 90-day outlook), encrypted self-Person seed, encrypted home-location decrypt, straggler re-encrypt → 0 stragglers.
...
### 9.3 Cross-household trips = a deliberate plaintext exception (decided 2026-07-07)

A trip can be shared with **collaborators outside the owner's household**, who hold no HDK. Rather than build cross-household key wrapping, the product accepts that **a shared trip's content stays plaintext** so those collaborators can read it — trips are inherently a cross-boundary sharing feature. Decision: *encrypt-private, plaintext-when-shared* (with **lazy re-encrypt** on unshare — a trip that drops to zero collaborators stays plaintext until its next edit re-seals it).

- **"Shared" = an outside email is on the trip (`sharedWithOutside`, an invitation pending) OR ≥1 collaborator has joined.** Disabling sharing clears both. Centralized in `services/tripSharing.js` (`isTripShared`, `sharedTripIds`, `excludeSharedFilter`; unit-tested). **Sharing is now by email invitation, not a share code** (updated 2026-07-12): the owner keeps a list of outside emails on the trip; each new email gets a `TripInvitation` (+ email; lazily claimed on late registration; unique per trip+email), the recipient accepts from their Invitations inbox to land in `collaborators`, and removing the email revokes the invitation + unseats the collaborator — the exact mechanics of shared calendars (§9.5). The old `shareCode` / `POST /trips/join` code flow is gone.
- **Drop safety (built 2026-07-07):** shared trips + their `TripItem`s are exempt from the plaintext drop — `scripts/dropPlaintext.js` skips them in *both* the straggler check (a shared trip legitimately lacks `enc`, so it must not abort the drop) and the commit `$unset` (never null their plaintext). `GET /household/e2ee/stragglers` excludes them too, so the owner's re-encrypt pass never seals a shared trip.
- **Sharing pre-drop just works** — dual-write keeps plaintext present, so a collaborator reads it today with no extra step.
- **Decrypt-on-share (built).** Post-drop a *private* trip is ciphertext-only; to share it the owner's device decrypts + re-saves the trip and its items as plaintext (clearing `enc`). This now fires when the **first outside email** is added: `PUT /trips/:id/share` returns `409 decrypt_required` on an `e2eeActive` household when the trip isn't already shared and a new email is being added; the mobile client catches it, decrypts via `openRecord`, and retries with `{ emails, decrypted }`. Steady-state edits to an already-shared trip **strip `enc`** (the `isTripShared` write guard), so a re-seal can't reintroduce ciphertext a collaborator can't read.
- **Attachments** on a shared trip must likewise be plaintext (ties into the still-open trip-attachments item above).

### 9.4 Event invitations = a plaintext *snapshot*, not a plaintext record (built 2026-07-08)

Calendar events can be shared **outside the household by email** (`routes/invitations.js`, `models/EventInvitation.js`). Unlike shared trips, the source `CalendarEvent` is **never exempted from E2EE**: the inviting client posts a **plaintext snapshot** of the event (decrypted on-device) inside the `EventInvitation`, and that snapshot is the whole contract — it feeds the invite email + `.ics` attachment, the recipient's in-app Invitations screen, and (on accept) an independent **copy** created under the recipient's own `userId`. The original stays sealed; edits after inviting don't propagate. `EventInvitation` carries no `enc` field and is outside `CONTENT_MODELS`, so the drop machinery ignores it. The accept-copy is created plaintext server-side (same as any dual-write-era server write) and re-seals via the recipient's lazy re-encrypt on next edit. Works pre- and post-drop with no extra steps: the client always supplies the snapshot (server falls back to source plaintext only while dual-write still carries it). The copy back-links its invitation (`CalendarEvent.invitationId` ↔ `EventInvitation.acceptedEventId`), which drives the guest's "Leave event" (copy deleted, invitation → `left`) and the organizer's invitee list + revoke (`GET /invitations/sent` is gated on the source event's household scope, so a recipient never sees the other invitees). Invited copies are **read-only for the recipient household** — `PUT`/`DELETE /calendar/events/:id` return 403 on any event carrying `invitationId` (leave is the only exit), so a copy never re-seals via the edit path; the straggler pass seals it instead through `/e2ee/seal`, which bypasses that guard by design. Accept/decline push-notifies the sender (`services/notify`).

**Guest-list visibility (added 2026-07-10):** the organizer controls whether invitees can see who else is invited. `CalendarEvent.guestListVisible` (Boolean, default true — events predating the flag read as visible) is a **plaintext scope/permission field like `alertAudience`**: it is not in `DROP_FIELDS`, so it survives the §9 drop, which the server needs because it *enforces* the flag — `GET /invitations/:id/guests` (addressed-to-me gate, same as accept/decline) looks up the source event and, only when the flag allows, returns the sibling invitations' `toEmail/toPhone/status` (an explicit `select` keeps `shareToken` out). The flag is kept **out of the sealed content subset** (the mobile form otherwise seals its whole payload): it is toggled on the Invitees screen — a plaintext-only `PUT` for saved events, or via the create payload for drafts — so a stale `enc` merge can never revert it. If the source event is deleted, the guest list reads as hidden.

**SMS invites (added 2026-07-10):** invitations can also be addressed by `toPhone`. The server only records the invitation — the organizer's device sends the actual text (prefilled Messages composer) carrying a **public capability URL** `GET /invitations/public/:id/ics?k=<shareToken>` (unauthenticated, token compared timing-safe) that serves the snapshot's `.ics`. Exposure is equivalent to the email path's `.ics` attachment (the snapshot is already a deliberate plaintext hand-off); the URL is a bearer secret with the same trust model as the email itself, and revoking the invitation kills the link. Phone invites never resolve to an account (accounts are keyed by email), so they have no in-app accept/decline lifecycle — they stay `pending` and only the organizer's revoke removes them.

### 9.5 Custom calendars = plaintext metadata; outside sharing = a plaintext *feed* (built 2026-07-10)

Users can create their own calendars (mobile Calendars → Add Calendar; `routes/calendars.js`, `models/CustomCalendar.js`). A calendar record is **deliberately plaintext metadata** (same lane as the household name, §1.4): `name`, `color`, `alertsEnabled`, and the sharing tiers — `sharedWithHousehold`, `sharedWith` (member user ids), `sharedWithOutside` (emails). The record's `key` (`custom-<slug>`, client-minted) is what events reference via `CalendarEvent.calendarType`, whose validator now accepts it alongside the built-ins. `CustomCalendar` carries no `enc` field and sits outside `CONTENT_MODELS` — the drop machinery ignores it. If calendar names are later deemed content, this model needs an enc migration of its own.

- **Access filtering is client-side by design:** the server's list endpoint scopes which *calendar records* a member sees (own + household-shared + member-shared + collaborator, `mine` flag for creator-only edit), but *event* filtering can't live server-side post-drop. `lib/calendarData.loadCalendarData` — the one chokepoint every calendar view and the reminder scheduler load through — hides events whose `calendarType` is a `custom-` id outside the user's accessible set. Within-household this is cosmetic, not cryptographic: every member holds the HDK.
- Mobile hydrates `calendarPrefs` from the server with AsyncStorage as a warm-start cache (one-time upload migrates pre-server local calendars, guarded by `hc_custom_calendars_synced`).
- Per-calendar `alertsEnabled` gates the on-device reminder scheduler (`computeReminders` muted-set).

**Outside-household sharing (`models/CalendarInvitation.js`)** follows the *shared-trip* lane (§9.3), not the event-invitation lane (§9.4), because a shared calendar is an **ongoing feed**, not a one-shot snapshot. Each email on `sharedWithOutside` gets a `CalendarInvitation` (+ notification email; lazily claimed if the address registers later, unique per calendar+email); accepting puts the recipient in the calendar's `collaborators`, which grants **live access**: the calendar appears in their `GET /calendars` (sharing details stripped — a collaborator never sees member ids or the other outside emails, mirroring §9.4's invitee privacy), and `fetchCalendarSources` widens their event scope by `calendarType` key exactly like trips widen by `collaborators`. Removing the email (or deleting the calendar, or the recipient declining — including after accepting) revokes: invitation deleted/updated, collaborator unseated, feed gone.

**Access levels (added 2026-07-11):** every share carries **View Only or Full Access** — `householdAccess` for household-wide sharing, per-entry on `sharedWith`/`sharedWithOutside`/`collaborators` (legacy plain-id rows normalize in `services/calendarSharing.js`: members → 'full', their historical capability; outside → 'view'). Event writes enforce it server-side via `eventAuthz` (routes/calendar.js) on create/edit/delete/move: View Only reads come back flagged `readOnly` (response-only; drives the client's read-only view) and writes 403. **Full Access outside collaborators may create/edit/delete events on the shared calendar**; their events are created under their own userId, so the owning household's `fetchCalendarSources` also widens by its *owned* calendar keys to see them. Post-drop caveat: for household-internal calendars `eventAuthz` needs plaintext `calendarType`, so (like the client-side visibility filter) it's household-cosmetic then; the outside boundary stays enforceable because shared-calendar events are drop-exempt.

- **E2EE consequences (mirrors tripSharing, centralized in `services/calendarSharing.js`):** events on an outside-shared calendar ("outside-shared" = ≥1 outside email OR ≥1 joined collaborator) must stay **plaintext** — collaborators hold no HDK. The drop script and `GET /household/e2ee/stragglers` both exempt them (`excludeOutsideCalendarFilter`); un-sharing returns the events to normal private records via lazy re-encrypt.
- **Fail-safe pre-decrypt-on-share:** on an `e2eeActive` household, adding outside emails returns `409 decrypt_required` (same contract as `POST /trips/:id/share`) — the client explains outside sharing isn't available for E2EE households yet. Deferred, like trips: the decrypt-on-share client step, and a steady-state write rule keeping events on shared calendars plaintext post-drop.
...
**Phase 6 — Storage-mode toggle + download-first + 7-day purge** · *M* · ⏳ **FIRST SLICE DONE** (2026-07-07; build/test-verified; destructive purge deferred as dry-run)
...
*Delivered (dry-run, mobile-first — nothing destructive runs):*
- **Server:** `User` §4.1 fields (`storageMode`, `cloudDeletionScheduledAt/State`, `localReplicaVerifiedAt`, `localReplicaManifestHash`) + sparse `{cloudDeletionScheduledAt}` index. Pure **`services/cloudDeletion.js`** — `purgeDateFrom` (now+7d), `canGoLocal` (solo ⇔ memberCount≤1), `isDueForPurge`, `buildManifest`/`manifestsMatch` (download-first completeness proof: per-collection counts + a `cyrb53` fingerprint over `collection:id:updatedAt`), `isReplicaVerificationFresh`; **7 tests + a cyrb53 fixed-vector pin** (server suite **36**). **`routes/storage.js`** (`GET /storage` state+`canGoLocal`; `POST /switch-to-local` verifies the client manifest against the server's own over 8 collections, then — only on match, atomically — sets local + schedules the purge + AuditLog `deletion_scheduled` + email; `POST /switch-to-cloud` cancels → AuditLog `deletion_canceled` + email). **`services/mailer.js`** — nodemailer with an `isConfigured()` guard (no-op + log when SMTP unset), scheduled/canceled/purged templates. **`scheduler.js`** — daily 04:00 `runCloudPurgeSweep`: finds due scheduled users; **dry-run logs only** unless `CLOUD_PURGE_LIVE==='true'`, in which case it deletes ciphertext across the covered collections, marks `purged`, audits, emails.
- **Mobile:** `storageApi` + `lib/storageMode.ts` (`replicateAndBuildManifest` — download-first fetch of all 8 covered collections into the replica, `cyrb53`/`buildManifest` **mirrored exactly** from the server, pinned by the server fixed-vector test), `lib/storageState.ts` (shared subscriber store + `daysUntil`). `PrivacyScreen` wires the toggle: solo guard message for members (§6.1), blocking confirmation dialog (single-device / no-recovery / 7-day undo / email), download-first→schedule, and cloud-undo. App-wide `StorageBanner` (persistent countdown + "Keep in cloud" undo) mounted above the navigator.
*Web client UX (2026-07-07, build-verified):* `storageMode.js` (download-first over the IndexedDB replica, `cyrb53`/`buildManifest` mirrored) + `storageState.js` + `StorageBanner.vue` (mounted in `App.vue`) + `StorageSection.vue` (profile hub `Data storage`) — the same solo-guard → confirm → download-first → schedule → undo flow as mobile.
*Honest scope / deferred (per the scope decision — unverifiable in this env):* the **destructive purge is dry-run only** (gated behind `CLOUD_PURGE_LIVE`); real email **send** is a guarded no-op (needs SMTP env); the manifest covers the 8 record collections but **not attachments** or status/range edge cases, so it's a partial local copy — both reasons the live delete stays gated until staging + a fuller replica. local→household / household→solo transitions (§6.4/§6.5) ride on the existing join/leave flows; not specially wired here.
